...
# Apartado para ingresar información del usuario
st.markdown(
    """
...
# Entrada de texto para el número de teléfono
phone_number = st.text_input("Número telefónico:")
...
selected_etf1 = st.selectbox("Selecciona el primer ETF", etf_names)
selected_etf2 = st.selectbox("Selecciona el segundo ETF", etf_names)


# Verificar si los ETFs seleccionados son diferentes
if selected_etf1 == selected_etf2:
    st.error("Debes elegir un ETF diferente al primero.")
else:
    ...
    st.write(f"**Descripción de {selected_etf1}:**")
    ...
    st.write(f"**Descripción de {selected_etf2}:**")
    st.markdown(f"<p style='font-size:18px'>{description2}</p>", unsafe_allow_html=True)


    # Crear una selección para el período de tiempo
    period_options = ["10 años", "5 años", "3 años", "1 año", "6 meses", "3 meses", "1 mes"]
    selected_period = st.selectbox("Selecciona el periodo para comparar los ETFs", period_options)
    ...
    # Asegúrate de que aquí empieza el cálculo y la visualización de datos
...
# Graficar los precios de cierre
    ...
    plt.plot(data1['Close'], label=f'Precio al cierre {selected_etf1}', color='blue')
    plt.plot(data2['Close'], label=f'Precio al cierre {selected_etf2}', color='orange')
    plt.title(f'Comparación de precios entre {selected_etf1} y {selected_etf2} ({selected_period})')
    plt.xlabel('Fecha')
...
def mostrar_rendimiento(etf, performance):
    ...
    st.markdown(f"<span style='color:{color}; font-size: 20px'><b>Rendimiento de {etf} in the selected period:</b> {performance:.2f}%</span>", unsafe_allow_html=True)
...
data1['Monthly Return'] = data1['Close'].resample('M').ffill().pct_change()
data2['Monthly Return'] = data2['Close'].resample('M').ffill().pct_change()


# Calcular la volatilidad mensual como la desviación estándar de los rendimientos mensuales
volatility1 = data1['Monthly Return'].std() * 100  # en porcentaje
volatility2 = data2['Monthly Return'].std() * 100  # en porcentaje


# Mostrar la volatilidad mensual en Streamlit
st.metric(label=f"Volatilidad mensual de {selected_etf1}", value=f"{volatility1:.2f}%")
st.metric(label=f"Volatilidad mensual de {selected_etf2}", value=f"{volatility2:.2f}%")
...
st.markdown(
    ...
    <div style="background-color:#e0e0e0;padding:10px;border-radius:8px;margin-top:10px">
        <p style="font-size:18px;font-weight:bold;color:#333;text-align:center;">
            Escribe la cantidad que deseas invertir en cada ETF
        </p>
    </div>
    """,
    unsafe_allow_html=True
